...
function AccountsSidebar({ isOpen, onClose, isCollapsed }) {
  ...
  const accountsNavItems = [
    {
      section: null,
      items: [{ icon: LayoutDashboard, label: "Dashboard", path: "/accounts/dashboard" }],
    },
    {
      section: "Finance Workspace",
      items: [
        { icon: WalletCards, label: "Budget Overview", path: "/accounts/budget" },
        { icon: ReceiptIndianRupee, label: "Transactions", path: "/accounts/transactions" },
      ],
    },
  ];
            ...
            {group.items.map((item) => {
              ...
              return (
                <NavLink
                  ...
                  to={item.path}
                  end={item.path === "/accounts/dashboard"}
                  onClick={onClose}
