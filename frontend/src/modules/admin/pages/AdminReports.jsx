...
function AdminReports() {
  ...
  const fetchReports = async (isManual = false) => {
    ...
    try {
      const data = await reportService.getAdminFiles("report");
      setReports(data || []);
      if (isManual) {
        addNotification("Project reports list refreshed.", "info", 1800, false);
      }
    } catch (err) {
      console.error("Failed to load project reports:", err);
      if (isManual) {
        addNotification("Failed to refresh project reports.", "error", 1800, false);
      }
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const filteredReports = useMemo(() => {
    return safeReports.filter((report) => {
      ...
      const fileName = report.originalFileName || report.original_file_name || report.fileName || report.file_name || "";
      const uploaderName = report.uploadedByName || report.uploaded_by_name || "";
      const projectId = report.projectId || report.project_id || "";
      const teamId = report.teamId || report.team_id || "";
  ...
  }, [safeReports, search, statusFilter]);

  const stats = useMemo(
    () => ({
      total: safeReports.length,
      pending: safeReports.filter((r) => r && r.status === "PENDING").length,
      approved: safeReports.filter((r) => r && r.status === "APPROVED").length,
      rejected: safeReports.filter((r) => r && r.status === "REJECTED").length,
    }),
    [safeReports]
  );
  ...
  const handleStatusUpdate = async (fileId, newStatus, fileName) => {
    try {
      ...
      addNotification(`Report '${fileName}' marked as ${newStatus}.`, "success", 1800, false);
      await fetchReports();
    } catch (err) {
      console.error(err);
      addNotification("Failed to update report status.", "error", 1800, false);
    }
  };
  ...
  return (
    <main className="admin-page" style={{ padding: "28px", minHeight: "calc(100vh - 60px)", background: "#f8fafc" }}>
      {/* Header */}
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "16px", marginBottom: "24px" }}>
        <div>
          <h1 style={{ margin: 0, fontSize: "1.8rem", fontWeight: "800", color: "#0f172a", display: "flex", alignItems: "center", gap: "10px" }}>
            <BarChart3 size={26} color="#1d5cff" />
            Project Reports Management
          </h1>
          <p style={{ margin: "4px 0 0", color: "#64748b", fontSize: "0.95rem" }}>
            Review quarterly and annual project report submissions, verify details, and issue clearance.
          </p>
        </div>
      ...
      {/* KPI Stats Grid */}
      <section className="stats-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "20px", marginBottom: "28px" }}>
        <div style={{ background: "#ffffff", padding: "20px", borderRadius: "14px", border: "1px solid #e2e8f0" }}>
          <span style={{ fontSize: "0.82rem", fontWeight: "700", color: "#64748b", textTransform: "uppercase" }}>Total Reports</span>
        ...
        <div style={{ background: "#ffffff", padding: "20px", borderRadius: "14px", border: "1px solid #e2e8f0" }}>
          <span style={{ fontSize: "0.82rem", fontWeight: "700", color: "#166534", textTransform: "uppercase" }}>Approved Reports</span>
          <strong style={{ display: "block", fontSize: "1.6rem", color: "#166534", marginTop: "4px" }}>{stats.approved}</strong>
        </div>
        <div style={{ background: "#ffffff", padding: "20px", borderRadius: "14px", border: "1px solid #e2e8f0" }}>
          <span style={{ fontSize: "0.82rem", fontWeight: "700", color: "#991b1b", textTransform: "uppercase" }}>Rejected Reports</span>
      ...
      <div style={{ background: "#ffffff", border: "1px solid #e2e8f0", borderRadius: "14px", padding: "18px 22px", marginBottom: "28px", display: "flex", gap: "16px", flexWrap: "wrap", alignItems: "center", boxShadow: "0 4px 12px rgba(0,0,0,0.03)" }}>
        <div style={{ flex: 1, minWidth: "260px", position: "relative" }}>
          <input
            type="text"
            placeholder="Search by report name, uploader, project ID, team..."
            value={search}
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left", fontSize: "0.88rem" }}>
              <thead>
                <tr style={{ background: "#f8fafc", borderBottom: "1px solid #e2e8f0", color: "#475569" }}>
                  <th style={{ padding: "14px 18px", fontWeight: "700" }}>Report Name</th>
              ...
              <tbody>
                {filteredReports.length > 0 ? (
                  filteredReports.map((report) => {
                    const fileDirectUrl = getFileUrl(report.filePath || report.file_path || report.url);
                    const displayName = report.originalFileName || report.original_file_name || report.fileName || report.file_name || "Project Report";
                    const uploaderName = report.uploadedByName || report.uploaded_by_name || "Team Member";
                    const uploaderEmail = report.uploadedByEmail || report.uploaded_by_email || "";
                    const createdAtVal = report.createdAt || report.created_at;
                    const dateStr = formatDateTime(createdAtVal);
                    const sizeBytes = report.fileSize ?? report.file_size ?? 0;
                    const projectIdVal = report.projectId || report.project_id || "N/A";
                    const teamIdVal = report.teamId || report.team_id || "N/A";
                        ...
                        <td style={{ padding: "14px 18px", textAlign: "center", whiteSpace: "nowrap" }}>
                          <div style={{ display: "flex", justifyContent: "center", gap: "8px", flexWrap: "nowrap" }}>
                            ...
                            {report.status !== "APPROVED" && (
                              <button
                                type="button"
                                onClick={() => handleStatusUpdate(report.id, "APPROVED", displayName)}
                            ...
                            {report.status !== "REJECTED" && (
                              <button
                                type="button"
                                onClick={() => handleStatusUpdate(report.id, "REJECTED", displayName)}
      ...
      {/* Preview Modal */}
      {previewFile && (() => {
        const fileName = previewFile.originalFileName || previewFile.original_file_name || previewFile.fileName || previewFile.file_name || "Report";
