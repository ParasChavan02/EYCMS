...
export default function AdminUCManagement() {
  ...
  // Fetch all transactions and files for real-time validation checks
  ...
  const updateSubmittedStatus = async (fileId, newStatus, fileName) => {
    const ucFile = submittedFiles.find(f => f.id === fileId);
    
    // Validation before approval check
    if (newStatus === "APPROVED" && ucFile) {
      const projId = ucFile.project_id || ucFile.projectId;
      
      const projTxns = allTransactions.filter(t => t.project_id === projId || t.project === projId);
      const pendingTxns = projTxns.filter(t => t.status !== "APPROVED");
      const unreconciledTxns = projTxns.filter(t => t.reconciliation_status !== "APPROVED" && t.reconciliation_status !== "LOCKED");
      
      const projBills = allFiles.filter(f => f.project_id === projId && f.category === "bill" && f.status !== "APPROVED");

      const warnings = [];
      if (pendingTxns.length > 0) warnings.push(`${pendingTxns.length} transactions are pending approval.`);
      if (unreconciledTxns.length > 0) warnings.push(`${unreconciledTxns.length} transactions are unreconciled.`);
      if (projBills.length > 0) warnings.push(`${projBills.length} bills are pending review.`);

      if (warnings.length > 0) {
        const confirmMsg = `WARNING: UC Pre-approval check failed:\n\n` + 
                           warnings.map(w => `- ${w}`).join("\n") + 
                           `\n\nAre you sure you want to bypass and force-approve this UC?`;
    ...
    try {
      await ucAdminService.updateSubmittedUCStatus(fileId, newStatus, remarks);
  ...
  const currentProjectUcVersions = useMemo(() => {
    ...
    const projId = previewFile.project_id || previewFile.projectId;
    return submittedFiles
      .filter(f => f.category === "uc" && (f.project_id === projId || f.projectId === projId))
      .sort((a, b) => new Date(a.created_at || a.createdAt) - new Date(b.created_at || b.createdAt))
      .map((f, i) => ({
        ...f,
        versionNum: i + 1
      }));
  }, [previewFile, submittedFiles]);

  // Find related transactions for validation
  ...
  return (
    <main style={{ padding: 28, minHeight: "calc(100vh - 60px)", background: "linear-gradient(180deg, #f8fafc 0%, #eef6fb 100%)" }}>
      ...
      {previewFile && (() => {
        ...
        // Perform validations for selected project
        const pendingTxns = relatedTransactions.filter(t => t.status !== "APPROVED");
        const unreconciledTxns = relatedTransactions.filter(t => t.reconciliation_status !== "APPROVED" && t.reconciliation_status !== "LOCKED");
        const pendingBills = allFiles.filter(f => f.project_id === projId && f.category === "bill" && f.status !== "APPROVED");
        ...
        return (
          ...
          >
            ...
            >
              ...
              {/* LEFT COLUMN: PDF / Image preview with Zoom controls */}
              <div style={{ display: "flex", flexDirection: "column", background: "#f8fafc", borderRight: "1px solid #e2e8f0", height: "100%" }}>
                <div style={{ padding: "14px 20px", borderBottom: "1px solid #e2e8f0", display: "flex", justifyContent: "space-between", alignItems: "center", background: "#f1f5f9" }}>
                  <span style={{ fontSize: "12px", fontWeight: "700", color: "#334155" }}>Document Viewer</span>
                  <div style={{ display: "flex", gap: "6px" }}>
                    <button onClick={() => setZoom(prev => Math.max(50, prev - 25))} className="btn-sm"><ZoomOut size={14} /></button>
                    <button onClick={() => setZoom(prev => Math.min(200, prev + 25))} className="btn-sm"><ZoomIn size={14} /></button>
                    <button onClick={() => setRotation(prev => (prev + 90) % 360)} className="btn-sm"><RotateCw size={14} /></button>
                  </div>
                </div>

                <div style={{ flexGrow: 1, display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", padding: "20px", position: "relative" }}>
                  {isImage ? (
                    <div style={{ width: "100%", height: "100%", overflow: "auto", display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <img src={fileDirectUrl} alt={fileName} style={{ maxWidth: "100%", maxHeight: "100%", transform: `scale(${zoom / 100}) rotate(${rotation}deg)`, transition: "transform 0.2s", objectFit: "contain" }} />
                    </div>
                  ) : isPdf ? (
                    <iframe src={fileDirectUrl} title={fileName} style={{ width: "100%", height: "100%", border: "none", background: "#ffffff" }} />
                  ) : (
                    <div style={{ textAlign: "center", padding: "40px", background: "#ffffff", borderRadius: 12, border: "1px solid #cbd5e1" }}>
                      <FileText size={52} color="#0f766e" style={{ marginBottom: 14 }} />
                      <h3>No Preview Available</h3>
                      <a href={fileDirectUrl} download={fileName} target="_blank" rel="noopener noreferrer" className="sp-btn sp-btn-primary">Download File</a>
                    </div>
                  )}
                </div>
              </div>

              {/* RIGHT COLUMN: Extensive reviews, related transactions, timelines, validations & approvals */}
              <div style={{ display: "flex", flexDirection: "column", height: "100%", background: "#ffffff" }}>
                ...
                <div style={{ flexGrow: 1, overflowY: "auto", padding: "20px", display: "flex", flexDirection: "column", gap: "16px" }}>
                  ...
                  {/* Pre-approval Validation alerts */}
                  <article style={{ background: "#fffbeb", border: "1px solid #fde68a", borderRadius: "10px", padding: "12px" }}>
                    <span style={{ fontSize: "11px", fontWeight: "700", color: "#b45309", display: "flex", alignItems: "center", gap: "6px", marginBottom: "6px" }}>
                      <AlertTriangle size={14} /> UC Pre-approval System Checks
                    </span>
                    <div style={{ display: "flex", flexDirection: "column", gap: "4px", fontSize: "11px", color: "#78350f" }}>
                      <div style={{ display: "flex", justifyContent: "space-between" }}>
                        <span>Pending Transactions Approval:</span>
                        <strong>{pendingTxns.length === 0 ? "✓ None" : `${pendingTxns.length} pending`}</strong>
                      </div>
                      <div style={{ display: "flex", justifyContent: "space-between" }}>
                        <span>Unreconciled Transactions:</span>
                        <strong>{unreconciledTxns.length === 0 ? "✓ Reconciled" : `${unreconciledTxns.length} unreconciled`}</strong>
                      </div>
                      <div style={{ display: "flex", justifyContent: "space-between" }}>
                        <span>Pending Review Bills:</span>
                        <strong>{pendingBills.length === 0 ? "✓ Clear" : `${pendingBills.length} pending`}</strong>
                      </div>
                    </div>
                  </article>

                  {/* Financial & Project summary card */}
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", background: "#f8fafc", padding: "12px", borderRadius: "8px", border: "1px solid #e2e8f0", fontSize: "11px" }}>
                    <div>
                      ...
                      <strong style={{ display: "block", color: "#1e293b", marginTop: "2px" }}>{projId || "Unassigned"}</strong>
                    </div>
                    <div>
                      <span style={{ color: "#64748b" }}>Reporting Status</span>
                      <strong style={{ display: "block", color: "#1d5cff", marginTop: "2px" }}>{previewFile.status}</strong>
                    </div>
                  </div>

                  {/* Related General Ledger Transactions */}
                  <div>
                    <span style={{ fontSize: "11px", fontWeight: "700", color: "#475569", display: "block", marginBottom: "6px" }}>Related Project Transactions ({relatedTransactions.length})</span>
                    <div className="table-wrapper" style={{ border: "1px solid #e2e8f0", borderRadius: "8px", maxHeight: "150px", overflowY: "auto" }}>
                      <table className="admin-table" style={{ fontSize: "11px" }}>
                        ...
                        <tbody>
                          {relatedTransactions.map((t) => (
                            <tr key={t.id}>
                              <td style={{ padding: "6px 8px", fontFamily: "monospace" }}>{t.id.substring(0, 8)}</td>
                              <td style={{ padding: "6px 8px" }}>{t.description}</td>
                              <td style={{ padding: "6px 8px" }}><span className={`status-badge ${t.status?.toLowerCase() === "approved" ? "approved" : "pending"}`} style={{ fontSize: "9px", padding: "2px 6px" }}>{t.status}</span></td>
                              <td style={{ padding: "6px 8px", textAlign: "right", fontWeight: "700" }}>{formatCurrency(t.amount)}</td>
                            </tr>
                          ))}
                          {!relatedTransactions.length && (
                            <tr><td colSpan={4} className="empty-state">No transaction ledger items for this project.</td></tr>
                          )}
                        </tbody>
                      </table>
                    </div>
                  ...
                  <div>
                    <span style={{ fontSize: "11px", fontWeight: "700", color: "#475569", display: "block", marginBottom: "6px" }}>UC Version History</span>
                    <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                      {currentProjectUcVersions.map(v => (
                        <div key={v.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 12px", border: "1px solid #e2e8f0", borderRadius: "6px", background: v.id === previewFile.id ? "#eff6ff" : "#ffffff", fontSize: "11px" }}>
                          <div>
                            <strong>Version {v.versionNum}</strong>
                            <span style={{ marginLeft: "8px", color: "#64748b" }}>{v.original_file_name || v.originalFileName}</span>
                          </div>
                          <span style={{ color: "#94a3b8" }}>{new Date(v.created_at || v.createdAt).toLocaleDateString()}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  {/* Audit Timeline */}
                  <div>
                    <span style={{ fontSize: "11px", fontWeight: "700", color: "#475569", display: "block", marginBottom: "6px" }}>Verification Audit Timeline</span>
                    <div style={{ display: "flex", flexDirection: "column", gap: "8px", fontSize: "10px", color: "#64748b" }}>
                      <div style={{ display: "flex", justifyContent: "space-between" }}>
                        <span>Uploaded by Fellow</span>
                        <strong>{dateStr}</strong>
                      </div>
                      {previewFile.admin_notes && (
                        <div style={{ background: "#f8fafc", padding: "6px", borderRadius: "6px", border: "1px solid #e2e8f0" }}>
                          <strong>Notes:</strong> {previewFile.admin_notes}
                        </div>
                      )}
                    </div>
                  </div>
                ...
                {/* Actions & Remarks Footer */}
                <div style={{ padding: "20px", borderTop: "1px solid #e2e8f0", background: "#f8fafc" }}>
                  <div style={{ marginBottom: "12px" }}>
                    <label style={{ fontSize: "11px", fontWeight: "700", color: "#475569", display: "block", marginBottom: "4px" }}>Audit Remarks / Verification Notes</label>
                    <textarea
                      placeholder="Add review remarks, revision requests, or pre-approval logs..."
                      value={remarks}
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px" }}>
                    <button onClick={() => updateSubmittedStatus(previewFile.id, "APPROVED", fileName)} className="btn-sm" style={{ background: "#dcfce7", color: "#166534", borderColor: "#bbf7d0", fontWeight: "700" }}>
                      ✓ Approve UC
                    </button>
                    <button onClick={() => updateSubmittedStatus(previewFile.id, "REJECTED", fileName)} className="btn-sm danger" style={{ fontWeight: "700" }}>
                      ✗ Reject UC
                    </button>
                    <button onClick={() => updateSubmittedStatus(previewFile.id, "REVISION_REQUESTED", fileName)} className="btn-sm" style={{ gridColumn: "span 2", borderColor: "#d97706", color: "#d97706", background: "#fffbeb" }}>
