...
export default function AdminUCManagement() {
  ...
  const fetchUCDocuments = async (isManual = false) => {
    ...
    try {
      const data = await reportService.getAdminFiles("uc");
      setUcFiles(data || []);
      if (isManual) {
        addNotification("UC records refreshed.", "info", 1800, false);
      }
    } catch (err) {
      console.error("Failed to load UC files:", err);
      if (isManual) {
        addNotification("Failed to refresh UC files.", "error", 1800, false);
      }
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const safeUcFiles = useMemo(() => {
    return Array.isArray(ucFiles) ? ucFiles : [];
  }, [ucFiles]);
  ...
  const getStatusBadge = (status) => {
    switch (status?.toUpperCase()) {
      ...
      default:
        return <span style={{ background: "#fef3c7", color: "#92400e", padding: "4px 10px", borderRadius: "8px", fontSize: "0.75rem", fontWeight: "700", display: "inline-flex", alignItems: "center", gap: "4px", whiteSpace: "nowrap" }}><Clock size={12} /> Pending Clearance</span>;
    }
  };
    ...
    <main style={{ padding: "28px", minHeight: "calc(100vh - 60px)", background: "#f8fafc" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "16px", marginBottom: "24px" }}>
        <div>
          <h1 style={{ margin: 0, fontSize: "1.8rem", fontWeight: "800", color: "#0f172a", display: "flex", alignItems: "center", gap: "10px" }}>
            <FileCheck size={26} color="#8b5cf6" />
            Utilization Certificate (UC) Management
          </h1>
          <p style={{ margin: "4px 0 0", color: "#64748b", fontSize: "0.95rem" }}>
            Audit, verify, and grant financial clearance for submitted Utilization Certificates across project teams.
          </p>
        </div>

        <button
          type="button"
          onClick={() => fetchUCDocuments(true)}
          disabled={refreshing}
          style={{
            display: "inline-flex",
            alignItems: "center",
        ...
        >
          ...
          {refreshing ? "Refreshing..." : "Refresh UC List"}
        </button>
      </div>
      ...
      <div style={{ background: "#ffffff", border: "1px solid #e2e8f0", borderRadius: "14px", padding: "18px 22px", marginBottom: "28px", display: "flex", gap: "16px", flexWrap: "wrap", alignItems: "center", boxShadow: "0 4px 12px rgba(0,0,0,0.03)" }}>
        ...
        <div style={{ flex: 1, minWidth: "260px", position: "relative" }}>
          <input
            type="text"
            placeholder="Search by UC Name, Uploader Name, Project ID, Team..."
            value={search}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          ...
          >
            <option value="ALL">All UC Submissions</option>
        ...
        {/* Total stats pill */}
        <div style={{ marginLeft: "auto", background: "#f3e8ff", color: "#7e22ce", padding: "8px 14px", borderRadius: "8px", fontSize: "0.85rem", fontWeight: "700" }}>
          Total UC Submissions: {filteredUCs.length}
        </div>
      </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left", fontSize: "0.88rem" }}>
              <thead>
                <tr style={{ background: "#f8fafc", borderBottom: "1px solid #e2e8f0", color: "#475569" }}>
                  <th style={{ padding: "14px 18px", fontWeight: "700" }}>UC File Name</th>
                        ...
                        <td style={{ padding: "14px 18px", textAlign: "center", whiteSpace: "nowrap" }}>
                          <div style={{ display: "flex", justifyContent: "center", gap: "8px", flexWrap: "nowrap" }}>
                            ...
                            <a
                              ...
                              style={{ padding: "6px 12px", borderRadius: "8px", border: "none", background: "#f3e8ff", color: "#7e22ce", fontWeight: "600", fontSize: "0.78rem", cursor: "pointer", display: "inline-flex", alignItems: "center", gap: "4px", textDecoration: "none" }}
                            >
                              <Download size={14} />
                              Download
                            </a>
                            ...
                            {uc.status !== "APPROVED" && (
                              <button
                                type="button"
                                onClick={() => handleStatusUpdate(uc.id, "APPROVED", displayName)}
                            ...
                            {uc.status !== "REJECTED" && (
                              <button
                                type="button"
                                onClick={() => handleStatusUpdate(uc.id, "REJECTED", displayName)}
                ...
                {isImage ? (
                  ...
                  <div style={{ textAlign: "center", padding: "40px 20px", background: "#ffffff", borderRadius: "12px", border: "1px solid #e2e8f0", width: "100%", maxWidth: "500px" }}>
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      style={{ textDecoration: "none", padding: "10px 20px", borderRadius: "10px", background: "#8b5cf6", color: "#ffffff", fontWeight: "600", display: "inline-flex", alignItems: "center", gap: "8px" }}
                    >
                      <Download size={16} />
